...
const formatMessage = function (layer, phEntry, position, exceptionName, tailsLength, E) {
...
const getLayer = async function(modules, layerName, options, renderData, data, placeholderName, getters) {
    let renderedPlaceholder;
    if (options.mode && options.mode.fast && renderData.processedLayers.hasOwnProperty(placeholderName)) {
        renderedPlaceholder = renderData.processedLayers[placeholderName];
    } else {
        renderedPlaceholder = await internalRender(modules, layerName, renderData, data, options, getters);
        renderData.processedLayers[placeholderName] = renderedPlaceholder;
    }
...
const checkCountOfParts = function(numOfComponents, expect, layer, fullEntry, currentPosition, tailsLength, E) {
    if (numOfComponents < expect) {
        formatMessage(layer, fullEntry, currentPosition, 'TOO_FEW_PARTS', tailsLength, E);
    }
    if (numOfComponents > expect) {
        formatMessage(layer, fullEntry, currentPosition, 'TOO_MANY_PARTS', tailsLength, E);
    }
};
...
const internalRender = async function(modules, layerName, renderData, data, options, getters) {
    ...
    if (renderData.gettersKeys) {
        if (renderData.gettersKeys === 'single') {
            ...
            getter = findValue(layerName, renderData.gettersKeys, getters, renderData.isGettersMap);
        }
    }
    ...
    if (getter.value) {
        layer = await getter.value({...layer, data: data, options: options, getters: getters});
    }
    ...
    while ((match = regexp.exec(layer.value)) != null) {
        ...
        if (!phComponents) {
            formatMessage(layer, fullEntry, currentPosition, 'EMPTY_PLACEHOLDER', tailsLength, E);
        ...
        switch (operator) {
            case OPERATORS.INSERT:
                checkCountOfParts(numOfComponents, 2, layer, fullEntry, currentPosition, tailsLength, E);
                if (getVisibility(opStack)) {
                    renderedPlaceholder =
                        await getLayer(modules, phName, options, renderData, data, phName, getters);
            ...
            case OPERATORS.IF:
                ...
                renderedPlaceholder = getVisibility(opStack) ?
                    await getLayer(modules, phName, options, renderData, data, phName, getters) : false;
            ...
            case OPERATORS.ELSE:
                checkCountOfParts(numOfComponents, 1, layer, fullEntry, currentPosition, tailsLength, E);
