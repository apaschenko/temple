...
const getLayer = async function(args) {
    const {modules, layerName, renderData, phName} = args;
    let renderedPlaceholder;
    if (renderData.options.mode && renderData.options.mode.fast && renderData.processedLayers.hasOwnProperty(phName)) {
        renderedPlaceholder = renderData.processedLayers[phName];
    } else {
        renderedPlaceholder = await internalRender(renderData, layerName, modules);
        renderData.processedLayers[phName] = renderedPlaceholder;
    }
        ...
        switch (operator) {
            case OPERATORS.INSERT:
                ...
                if (getVisibility(opStack)) {
                    renderedPlaceholder =
                        await getLayer({
                            modules, layerName: phName, options, renderData, data, phName, getters
                        });
            ...
            case OPERATORS.IF:
                ...
                renderedPlaceholder = getVisibility(opStack) ?
                    await getLayer({modules, layerName: phName, options, renderData, data, phName, getters})
                    : false;
