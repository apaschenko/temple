...
class Lexeme {
    ...
    isNotProcessedYet() {
        return this.notProcessedYet;
    }

    markAsProcessed() {
        this.notProcessedYet = false;
        return this;
    }
...
class Accumulator {
    ...
    parse() {
        ...
        while (posInLayer < layerLength) {
            ...
            switch (status) {
                case STATUS.TEXT:
                    if (this.addValAndCheckOpenPh(symbol)) { // if the placeholder is opened
                ...
                case STATUS.PH_MAYBE_BLANK:
                    ...
                    if (closingStatus === CLOSE_STATUS.CLOSED) { // the placeholder is closed
                        ...
                        if (this.currentLexeme.getRootLexeme().getType() === LEXEME_TYPE.UNKNOWN) {
                            this.eFormatter(
                                'PH_IS_EMPTY',
                        ...
                        this.currentLexeme = new Lexeme(this).setType(LEXEME_TYPE.TEXT);
                        this.buffer = this.result = '';
                        status = STATUS.TEXT;
                        break;
                    }
                            ...
                            switch (delimiterType) {
                                ...
                                case DELIMITER_TYPE.BLANK:
                                    if (this.bracketStack.length > 0) {
                                        ...
                                        this.addLexeme(this.currentLexeme);
                                        if (! this.currentLexeme.isNotProcessedYet()) {
                                            this.currentLexeme = new Lexeme(
                                                this, INSERT_TYPE.SIBLING, this.currentLexeme.getRootLexeme()
                                            );
                                            this.currentLexeme.setType(LEXEME_TYPE.STRING).setDelimiter(symbol);
                                        }
                                    }
                                    break;

                                case DELIMITER_TYPE.OPEN_PARENTHESIS:
                                    this.currentLexeme.setKind(LEXEME_KIND.FUNCTION);
                                    this.addLexeme(this.currentLexeme);
                                    if (this.currentLexeme.isNotProcessedYet()) {
                                        this.eFormatter(
                                            'PH_PARENTHESES_WITHOUT_FUNC_NAME',
                                            this.layer.substring(this.phStartedAt, posInLayer),
                                            symbol,
                                            posInLayer
                                        );
                                        ...
                                        this.currentLexeme = new Lexeme(
                                            this,
                                            INSERT_TYPE.FIRST_ARG,
                                            this.currentLexeme.getRootLexeme(),
                                            this.currentLexeme.rootOfChain()
                                        );
                                    }
                                    break;
                            }
    ...
    addLexeme(lexeme) {
        ...
        let rootLexeme = lexeme.getRootLexeme();

        if (lexeme.getType() !== LEXEME_TYPE.UNKNOWN) {
            switch (lexeme.getInsertType()) {
                case INSERT_TYPE.CHILD:
                    lexeme.getParent().setChild(lexeme);
                    lexeme.markAsProcessed();
                    break;

                case INSERT_TYPE.FIRST_ARG:
                    lexeme.getParent().setFirstArg(lexeme);
                    lexeme.markAsProcessed();
                    break;
            }
        }

        // chains insert as sibling
        if (
            (rootOfChain.getType() !== LEXEME_TYPE.UNKNOWN)
            && rootOfChain.isNotProcessedYet()
            && rootOfChain.getPrev()
        ) {
            rootOfChain.getPrev().setNext(rootOfChain);
            rootOfChain.markAsProcessed();
        }

        if ((rootLexeme.getType() !== LEXEME_TYPE.UNKNOWN) && rootLexeme.isNotProcessedYet()) {
            this.layerList.push(rootLexeme);
            rootLexeme.markAsProcessed();
        }
    }
