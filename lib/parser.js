...
class Lexeme {
    constructor(willBeInsertedAs) {
        ...
        this.init(willBeInsertedAs);

        return this;
    }

    init(willBeInsertedAs) {
        this.type  = (willBeInsertedAs === INSERT_TYPE.TEXT) ? LEXEME_TYPE.TEXT : LEXEME_TYPE.UNKNOWN;
        this.kind  = LEXEME_KIND.UNKNOWN;
...
class Accumulator {
    ...
    parse() {
        ...
        let posInLayer = 0;
        this.status = STATUS.TEXT;
        this.currentLexeme = new Lexeme(INSERT_TYPE.TEXT).setType(LEXEME_TYPE.TEXT);
        ...
        while (posInLayer < layerLength) {
            ...
            switch (status) {
                    ...
                    if (symbol === this.openPh[this.buffer.length]) {
                        if (this.buffer.length === this.openPhMaxPosition) {
                            if (this.result.length > 0) {
                                ...
                                this.addLexemeAndCreateNew(INSERT_TYPE.ROOT);
                            } else {
                                this.currentLexeme.init(INSERT_TYPE.ROOT);
                            }
                            rootLexeme = this.currentLexeme;
                ...
                case STATUS.PH_REGULAR:
                    ...
                    // Second: Processing symbol in regular mode
                                    ...
                                    case DELIMITER_TYPE.BLANK:
                                        if (this.bracketStack.length === 0) {
                                            ...
                                            this.currentLexeme.setType(LEXEME_TYPE.STRING)
                                                .setTerminator(symbol).setPrev(this.getLastTopLevelLexeme(rootLexeme));
                                        } else {
                                            this.eFormatter(
    ...
    addLexemeAndCreateNew(newInsertType) {
        let isNotProcessed = true;
        let prevLexeme = this.currentLexeme;

        if (prevLexeme.getLength() > 0) {
