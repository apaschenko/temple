...
class Accumulator {
    ...
    parse() {
        ...
        while (posInLayer < layerLength) {
            ...
            if (this.status === STATUS.TEXT) {
                if (this.addValAndCheckOpenPh(symbol, posInLayer)) { // if the placeholder is opened
                    this.currentLexeme = new Lexeme(this, INSERT_TYPE.ROOT);
            ...
            } else { // status === STATUS.PLACEHOLDER
                if (this.checkClosePh(symbol, posInLayer)) {
                    break;
                }
                ...
                if (BLANKS.includes(symbol)) {
                    ...
                    switch (this.currentLexeme.getType()) {
                        case LEXEME_TYPE.UNKNOWN:
                            ...
                            this.currentLexeme.setDelimiter(symbol);
                            break;
            ...
            } // end of status === STATUS.PLACEHOLDER
    ...
    addLexeme(lexeme) {
        let isNotProcessed = true;
        if ((lexeme.getLength() > 0) && lexeme.isNotProcessedYet()) {
    ...
    addValAndCheckOpenPh(symbol, posInLayer) {
    ...
    checkClosePh(symbol, posInLayer) {
        ...
        if ((this.lastCheckedPos < posInLayer) && (symbol === this.closePh[bufferLength])) {
            if (bufferLength === this.closePhMaxPosition) {
                ...
                if (this.currentLexeme.getRootLexeme().isNotProcessedYet()) {
                    this.eFormatter(
                        'PH_IS_EMPTY',
                        this.layer.substring(this.phStartedAt, posInLayer),
                        symbol,
