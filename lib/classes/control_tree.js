...
class ControlTree {
    constructor(options, layerName, layer) {
    ...
    formatSingleError(error) {
        const {lineSeparator, indentation} = this.options.printError;
        let err = { ...error };
        let output = `Error: ${ErrorName[err.name] || err.name}${lineSeparator}`;
        delete err.name;
        let keys = Object.keys(err);
        for (let key of keys) {
            output += `${indentation}${key}: ${err[key]}${lineSeparator}`;
        }
...
console.log('\nErr -> ', error, '\noutput: --->\n',output)
        ...
        if (output.length > 0) {
            output = `${lineSeparator}${lineSeparator}`
                ...
                + `${lineSeparator}${ErrorName.HEADER}`
